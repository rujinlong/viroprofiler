...
workflow CONTIGANNO {
    main:
    // Input validation lives in this body rather than at file scope: the strict script
    // syntax allows no statements outside a declaration.
    ...
    emit:
    // Contig annotation runs no MULTIQC, so there is no report for the completion e-mail
    // to attach. The placeholder keeps the emit signature the same as VIROPROFILER's, so
    // main.nf needs no special case.
    multiqc_report = channel.value([])
}
