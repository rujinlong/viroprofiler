...
`bin/` contains 17 Python/R/shell scripts called by processes (e.g., `run_checkv.sh`, `parse_mmseqsTaxa.py`, `create_tse.r`, `genomad_contig_table.py`).
...
Optional modules controlled by `use_*` flags: `use_dram` (true), `use_iphop` (true), `use_checkamg` (true), `use_vibrant` (true), `use_eggnog` (false), `use_kraken2` (false), `use_phamb` (false), `use_abricate` (false), `use_decontam` (false).
